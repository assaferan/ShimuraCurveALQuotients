...
negtab := [ AssociativeArray() : w in words ];
for wi->w in words do
    ...
    for ri->r in monos do
        ...
        a0tab[wi][ri] := k0 * c0;
        // negative-exponent harvest: Q-exponent (L + j)/(24W) for j = 0..-L-1 (all < 0)
        for j in [0..(-L)-1] do
            cj := Coefficient(produ, j);
            if cj eq 0 then continue; end if;
            ex := (L + j)/(24*W);          // exact rational Q-exponent, < 0
            if not IsDefined(negtab[wi], ex) then negtab[wi][ex] := [ CC!0 : m in monos ]; end if;
            negtab[wi][ex][ri] := k0 * cj;
        end for;
    end for;
end for;
...
for k in keys do
    ...
    // ---- principal-part scan: F's coefficients at ALL cosets, negative exponents ----
    // FP[gamma-index][exponent] accumulated over cosets
    FP := AssociativeArray();
    for wi->w in words do
        exps := Sort(Setseq(Keys(negtab[wi])));
        if #exps eq 0 then continue; end if;
        rv := VVRhoInvE0FFT(fftdata, w);
        for ex in exps do
            row := negtab[wi][ex];
            aw := &+[ CC | f`coeffs[r] * row[Index(monos, r)] : r in Exponents(f)
                     | row[Index(monos, r)] ne CC!0 ];
            if Abs(aw) lt 10^(-40) then continue; end if;
            for i in [1..#elts] do
                if Abs(rv[i]) lt 10^(-40) then continue; end if;
                key := <i, ex>;
                if not IsDefined(FP, key) then FP[key] := CC!0; end if;
                FP[key] +:= rv[i] * aw;
            end for;
        end for;
    end for;
    printf "  PRINCIPAL PART of F (|coeff| > 1e-30):\n";
    pps := Sort([ k2 : k2 in Keys(FP) | Abs(FP[k2]) gt 10^(-30) ],
                func< x, y | x[2] eq y[2] select x[1] - y[1] else (x[2] lt y[2] select -1 else 1) >);
    for k2 in pps do
        i := k2[1]; ex := k2[2];
        printf "    gamma#%o q=%o  exp %o  coeff %o + %o i\n", i,
            qofidx(i), ex, RealField(10)!Re(FP[k2]), RealField(10)!Im(FP[k2]);
    end for;
end for;
quit;
