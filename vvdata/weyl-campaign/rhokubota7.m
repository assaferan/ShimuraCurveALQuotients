...
//
//   magma -b DD:=15 NN:=2 vvdata/weyl-campaign/rhokubota7.m
...
// the canonical 2-element t0 (first solution of B(t, y) = 2Q(y) on D[2])
...
for wi->w in words do
    ...
    // xi_meas from the max entry via the t0-character shape:
    // shape = sqrt(|Dc|/|D|) e(acQ(j) + B(x_c, j)) e(((1-a)/2) B(t0, z))
    mx := 1;
    ...
    tw := Valuation(c, 2) eq 1 select frac(((1 - a) div 2) * BB(t0, mx)) else 0;
    shape := Sqrt(CC!#dc/CC!n) *
             ee(frac(a*c*Qbar[jm] + (xc eq i0 select 0 else BB(xc, jm)) + tw));
    xim := rv[mx]/shape;
