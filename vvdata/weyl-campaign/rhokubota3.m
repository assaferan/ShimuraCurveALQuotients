// The even-c fix: rhokubota2 showed the 20 bad words at 15_2 (all v2(c) = 1)
// break because the fiber of x |-> c x + x_c carries NON-coherent phases, and
// the single-preimage assignment (take the first j that lands on z) is only
// valid when the fiber is coherent (odd c; and, by luck of the module, all of
// 10_7).  Here sv[z] is the SUM over the full fiber divided by |D_c|, which
// leaves every coherent word unchanged and is Stromberg's actual formula for
// the rest.  Scale drift, if any, shows up as a uniform |u| per v2(c).
//
//   magma -b DD:=15 NN:=2 vvdata/weyl-campaign/rhokubota3.m
...
for wi->w in words do
    ...
    for j in [1..n] do
        ...
        sv[z] +:= fac * ee(frac(a*c*Qbar[j] + (xc eq i0 select 0 else BB(xc, j)))) / #dc;
    end for;
    mx := 1;
