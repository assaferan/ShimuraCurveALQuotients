// The even-c fix, round 2 (rhoprobe2_evenc's verdict): the fiber phases ARE
// coherent; the defect of the naive single-term formula at v2(c) = 1 words is
// a LINEAR CHARACTER on the support,
//     rv[z] = u0 * e(B(w0, z)) * sv[z],   w0 = ((1-a)/2) * t0  mod D_c,
// where t0 is the canonical 2-element: the first solution of
// B(t, y) = 2 Q(y) mod 1 for all y in the 2-torsion D[2].  At 15_2 t0 =
// [0,0,1] and the law reproduced all 20 bad words (a = 3..29, c = -2,-6,-10);
// for v2(c) >= 2 the correction is trivial (exponent-2 two-part), and at
// bases with 2Q = 0 on D[2] (10_7) t0 = 0 so nothing changes.  This script
// applies the correction and re-scores u against the Kubota cocycle sign.
//
//   magma -b DD:=15 NN:=2 vvdata/weyl-campaign/rhokubota4.m
...
// the canonical 2-element t0: first solution of B(t, y) = 2 Q(y) mod 1 on D[2]
tor2 := [ i : i in [1..n] | IsZero(2*elts[i]) ];
t0 := 0;
for i in [1..n] do
    if forall{ j : j in tor2 | frac(2*Qbar[j] - BB(i, j)) eq 0 } then
        t0 := i; break;
    end if;
end for;
error if t0 eq 0, "no t0";
printf "t0 = %o (coords %o), trivial correction iff t0 = identity: %o\n",
    t0, Eltseq(elts[t0]), IsZero(elts[t0]);
...
for wi->w in words do
    ...
    for j in [1..n] do
        ...
        if sv[z] eq 0 then
            ...
            if Valuation(c, 2) eq 1 then
                sv[z] *:= ee(frac(((1 - a) div 2) * BB(t0, z)));
            end if;
        end if;
    end for;
    mx := 1;
