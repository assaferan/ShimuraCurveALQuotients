...
for k in keys do
    ...
    // per-coset dump for the Gauss-sum / cusp-class analysis: eta* = first nonzero iso
    est := isoidx[2];
    for wi->w in words do
        if Abs(a0w[wi]) lt 10^(-60) then continue; end if;
        g := VVWordMatrix(w);
        rv := VVRhoInvE0FFT(fftdata, w);
        cc := g[2][1] mod M; dd := g[2][2] mod M;
        printf "COSET %o c=%o d=%o gcd=%o a0=(%o,%o) rho=(%o,%o) contrib=(%o,%o)\n",
            k, cc, dd, GCD(cc, M),
            RealField(10)!Re(a0w[wi]), RealField(10)!Im(a0w[wi]),
            RealField(10)!Re(rv[est]), RealField(10)!Im(rv[est]),
            RealField(10)!Re(rv[est]*a0w[wi]), RealField(10)!Im(rv[est]*a0w[wi]);
    end for;
end for;
quit;
