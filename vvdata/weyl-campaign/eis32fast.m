// eis32fast: eis32 minus the BorcherdsForms setup (panel/keys not needed for
// the solve) -- rhov + E-constant table + the verbatim least-squares + readout.
//   magma -b DD:=210 NN:=1 EF:=<pool> eis32fast.m
...
for wi->w in words do
    ...
    rhov[wi] := rv[est];
    if wi mod 64 eq 0 then printf "rho %o/%o (len %o)\n", wi, nw, #w; end if;
end for;
printf "rhov done\n";
