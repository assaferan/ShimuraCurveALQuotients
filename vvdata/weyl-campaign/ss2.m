...
SC := 1;
if assigned SCL then SC := StringToInteger(SCL); end if;
Ld := ShimuraCurveLattice(D, N);
G := SC * ChangeRing(Ld`Q, Rationals());   // rescaled Gram; SC = 1 is A itself
Gi := G^(-1);
...
A, to_disc := Ldual2 / L2;
Qr := G;
printf "BASE %o %o SCALE %o  |A| = %o  invariants %o\n", D, N, SC, #A, Moduli(A);
