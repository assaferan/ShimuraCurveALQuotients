...
prec := 100;
if GetEnv("PREC") ne "" then prec := StringToInteger(GetEnv("PREC")); end if;
WriteStderr(Sprintf("Prec = %o\n", prec));
WriteStderr(Sprintf("IntegralSolution = %o\n", intsol));
covers, ws := AllEquationsAboveCovers(Xstar, curves : Prec := prec, IntegralSolution := intsol);
