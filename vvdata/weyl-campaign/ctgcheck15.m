...
// ---- GAP A: override beta with the GROSS COMBINATION'S pool coordinates ----
// The N>1 theorem says  -1/2 tb(5,2) + tb(5,6) - 1/2 tb(30,1)  has constant
// term rho at every coset.  ctpool15.log shows it lies in THIS pool (the 21
// holomorphic same-character weight-3/2 eta quotients at level 60), with the
// coordinates below.  Two things get tested by substituting it here:
//   * residG: its constant terms against rho, recomputed through the ETA path
//     (cosetleads/cosetfull/kappa) rather than the closed-form ctTheta path --
//     an independent confirmation of the theorem at 15_2;
//   * A_f, INTERM, CONV: whether it can REPLACE E* in the residue pairing.
// Matching by VALUE, not index, so a reordered pool cannot silently shift them.
gvecs := [ [0,0,1,0,0,0,-1,0,1,2,0,0], [0,1,-2,-2,0,2,0,4,0,0,0,0],
           [0,2,0,-1,0,0,0,0,-2,0,7,-3], [0,2,0,-1,2,0,1,0,0,-1,0,0],
           [2,-1,0,0,0,4,0,-2,0,0,0,0], [2,0,-1,0,-1,0,0,0,0,0,5,-2],
           [4,0,0,-1,-2,0,3,0,0,-1,0,0], [6,-3,-4,0,0,6,0,-2,0,0,0,0] ];
gvals := [ Rationals() | 2, -12/5, -1, 1/2, -6/5, 6, 9/10, 4/5 ];
gbeta := [ CC | 0 : a in [1..nE] ];
for j in [1..#gvecs] do
    idx := Index(Epool, [ Integers() | x : x in gvecs[j] ]);
    error if idx eq 0, "Gross pool vector not found in Epool", j;
    gbeta[idx] := CC!gvals[j];
end for;
residG := Sqrt(&+[ Abs(&+[ gbeta[a]*Emat[a][wi] : a in [1..nE] ] - rhov[wi])^2 : wi in [1..nw] ]);
printf "GROSS resid vs rho (INDEPENDENT eta path) = %o\n", RealField(10)!residG;
printf "GROSS differs from the solved beta by %o (expected nonzero: they differ by a cusp form)\n",
    RealField(10)!Sqrt(&+[ Abs(gbeta[a]-beta[a])^2 : a in [1..nE] ]);
beta := gbeta;
printf "=== USING THE GROSS COMBINATION AS E* FROM HERE ON ===\n";
