...
multiplier := function(foo, f0, Ldata, D, N, mode)
    ...
    for j in [1..-Valuation(f0)] do
        ...
        for eta in mod_M_to_vecs[bucket] do T +:= contrib(eta, r, Rationals()!c); end for;
    end for;
    return -96 * T / (D*N);
end function;
...
for base in [<15,2>, <21,2>] do
    ...
    try
        ...
        for k in ks do
            ...
            m0 := multiplier(foo, f0, Ldata, D, N, 0);
            m1 := multiplier(foo, f0, Ldata, D, N, 1);
            m2 := multiplier(foo, f0, Ldata, D, N, 2);
            emit(Sprintf("  form[%-3o]: OLD = %-9o  dens(+Q,-j) = %-9o  dens(-Q,+j) = %-9o", k, m0, m1, m2));
        end for;
    catch e
