procedure test_FieldOfDefinition()
    ...
    // arXiv:1510.06193 v1, Appendix B (the journal version, Compositio 153 (2017), is revised and
    // may fix some misprints below).  Table numbers are arXiv v1's.  The expected field K is the
    // field generated by the printed coordinate.
    //
    ...
    //    Probed by hand, these agree with the code except Table 10 d=-1435, a [GY] misprint:
    //    s = -369/7 = x^2 gives Q(sqrt(-287)), not the printed Q(sqrt(-41)).
    ...
    for datum in data do
        ...
        // Membership checks only: [GY] need not list every CM point.
        ...
        for pt in cm do
            d, K := Explode(pt);
            // CMClassLists stops at class number 8.
            if ClassNumber(d) gt 8 then continue; end if;
            // At bd := 8 the scan's table has only fundamental discriminants.
            ...
            if AbsoluteDegree(K) eq 1 then
                ...
                // The elliptic points d = -3, -4 can have degree 2 on a proper subquotient:
                // [GY] Table 16, d = -3 on X_0^58(1)/<w_29> has field Q(sqrt(-3)).
    ...
    // [GY] arXiv v1 prints s = -9/25 (d=-267) and s = -27/49 (d=-708), refuted by Shimura
    // reciprocity on [GY]'s own equations.  -s is the unique rational s = a/b, |a|, b <= 400,
    // passing the same square conditions (asserted below), and agrees with the code.
    ...
    // Uniqueness of -s.  Pic(R) has exponent 2, so H_R is the genus field and q in Q is a square
    // in H_R iff its squarefree part is a product of prime discriminants dividing d (spot-checked
    // below).  Search all s = a/b, gcd(a,b) = 1, |a| <= 400, 1 <= b <= 400.
