...
defect of \eqref{eq:oldform}, is where the computation currently stops. The discrepancy is
exactly the shape of the harmonic completion: the $\xi$-image of the weight-$3/2$ Eisenstein
series is a weight-$1/2$ theta on $A$ (supported, by Serre--Stark, on the isotropic lines ---
the same level-$N$ structure that carries everything else in this note), and its contribution to
the pairing is, for weakly holomorphic input, a \emph{finite} Weyl-vector-type functional of the
principal part in the sense of \cite[\S10]{Borcherds}. Calibrating that functional --- for which
the $\mathrm{mult}=0$ forms give equations free of every other unknown --- is the immediate next
step. We note the required care: the lattices here are anisotropic over $\Q$, so the unfolding
of \cite[\S10]{Borcherds} does not apply verbatim and the evaluation must proceed through the
