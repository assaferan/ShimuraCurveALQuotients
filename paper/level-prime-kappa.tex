...
Group the cosets by the class $g=\gcd(c,M)$ of their lower-left entry. On every base dumped
($X_0^{10}(7)$, $X_0^{15}(2)$, $X_0^{21}(2)$, $X_0^{33}(2)$, $X_0^{35}(2)$, $X_0^{6}(11)$;
570 checks in \texttt{vvdata/weyl-campaign/cuspclass2.py}) the per-coset product
$(\rho(w^{-1})e_0)_{\eta^*}\,a_0(f\vert_{1/2}w)$ is \emph{constant on each class}, for every
form: the eta-multiplier twist of $a_0$ along a class is always exactly cancelled by the
Weil-representation phase. Hence
\[
  c_{\eta^*}(0)\;=\;\sum_{g\mid M}\frac{M}{g}\,
  \bigl(\rho(w_g^{-1})e_0\bigr)_{\eta^*}\,a_0(f\vert_{1/2}w_g)
\]
at one canonical coset $w_g$ per class. The $\rho$-entry obeys base-independent laws: the
class is dead iff $g\equiv2\bmod4$ (no eta-quotient constant term) or $N\mid g$ (the
$e_{\eta^*}$ entry vanishes); $\lvert\rho\rvert=g\sqrt2/M$ at odd $g$ and $g/M$ at $4\mid g$
(real, with sign $\mu(g/4)$); and at odd $g$ the phase is $e(\theta_g)$ with, in the
canonical metaplectic normalisation,
\[
  8\theta_g\;\equiv\;2+4\,r(g)-4\,\omega(g)+\arg_8 G_2(-g)\pmod 8,\qquad
  \arg_8 G_2(x)=x+4\,\bigl[\bigl(\tfrac2x\bigr)=-1\bigr],
\]
where $r(g)=\#\{p\mid g:p\equiv3\bmod4\}$ and $\omega(g)$ is the number of prime factors
--- derived from Str\"omberg's explicit formula for the Weil representation
\cite{Stromberg2013} instantiated on the Jordan decomposition of $A$: the anisotropic
pairs at $p\mid D$ each contribute $-1$, the hyperbolic pairs at $p\mid N$ contribute
$+1$, and the base-independence is itself a theorem (the $2$-adic genus flips sign
exactly when $2\mid D$, while $\omega_{\mathrm{odd}}(D)$ flips parity exactly then,
because an indefinite quaternion algebra ramifies at an even number of primes; the two
cancel). The formula reproduces all nine measured classes; on $\omega(g)\le2$ --- every
measurable class on these bases --- it coincides with the symbol form
$1+6r(g)+4s(g)$, $s(g)=\#\{p<q\mid g:(\tfrac pq)(\tfrac qp)=+1\}$, but the two laws
diverge from $\omega(g)=3$ onward, so the symbol form is only the low-complexity shadow
of the derived one (\texttt{rho5.m}, \texttt{thetag-derivation.md}). The pinned constants of the evaluation are
themselves closed-form: they equal $\zeta_w\prod_d\bigl[\varepsilon(g_d)\,
e(b_d/24e_d)\,e_d^{-1/2}\bigr]^{r_d}$ with $\varepsilon$ the Dedekind-sum eta multiplier and
$\zeta_w$ the metaplectic cocycle unit of the word, certified on all $144$ cosets of
$X_0^{15}(2)$ and all $288$ of $X_0^{10}(7)$ (\texttt{cusp4.m}), so nothing in the assembly
remains numerical.
