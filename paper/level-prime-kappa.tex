...
is entirely mass normalisation; a tenth base, the first with composite $s$, is preregistered
against three competing extensions. Section~\ref{sec:open} states what remains open.
...
M_{3/2}(\Gamma_0(4DN))$ is exactly rational. Three constants are base-independent across
all nine bases: the Eisenstein dimension of $M_{3/2}(\Gamma_0(4DN))$ is $11$, the rank of
the constants map is $7$, and the aliasing kernel of the Gross-average fit has dimension
$6$.
\end{observation}

\begin{observation}[the $s$-law; nine bases]\label{obs:slaw}
On every base, $E_{\mathrm{eis}}$ lies in the span of the $\bar\theta(D',R)$ over the
...
\]
an identity verified on all nine bases; the second factor of the middle mass ratio is
multiplicative, $\prod_{p\mid s}(p+1)/2$, over the primes of $s$. The bracket depends on
the base only through $N$, and the fitted ``$s$-law'' is exactly the mass normalisation
of a fixed Siegel--Weil combination.
\end{observation}
...
required. Two normalisations remain open: the overall scale ($-2$ against the prime-case
$-1/(s-1)=-1$), and the composite-$D'$ genus masses, which carry a factor
$2^{\omega(D')-1}$ relative to the prime-case formula of Observation~\ref{obs:mass} ---
both now constraints on the derivation rather than free parameters.
