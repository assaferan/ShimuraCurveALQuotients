...
only $N=1$. So the step on which everything here turns is unverified precisely in the range where we
claim it to be incomplete. Constructing such a check --- for instance by comparing against a
published CM value on a base with $N>1$ and a nonvanishing correction, of which
$X_0^{14}(5)$ with $s(\tau_{-280})=5/16$ \cite[Ex.~36]{GY} appears to be the only available
instance --- would be the natural next step.
