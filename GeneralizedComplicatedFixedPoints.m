...
function IsTrivialModGamma0(M, L)
    e := Eltseq(M);
...
W_odd (the w in W coprime to p) together with the non-AL modular involution
V_p = S_p w_pPart S_p^-1 at p = 2 (V2, S2 = [2,1;0,2]; V3 is excluded, see below).  Via the
isomorphism C = X/W = X/G the involution w_pPart on X/G plays the Prop 6
...
count, namely (1/#W) times the sum over w in W of NumFixedPoints of V_p*w on X, to equal 4.}
    ...
    for idx->vname in all_names do
        ...
        // G = <W_odd, V_p> must equal S_p W S_p^-1: S_p must commute with every w in W_odd
        // (modulo Q^* Gamma_0(DN)), and G must be elementary abelian of order #W.
        M2Q := MatrixAlgebra(Rationals(), 2);
        Sp := M2Q![p, 1, 0, p];
        Vq := M2Q!V;
        Wmats := [M2Q!al_matrix(w, DN) : w in Wodd];
        if not &and[IsTrivialModGamma0(Sp*A*Sp^-1*A^-1, DN) : A in Wmats] then continue; end if;
        Gmats := Wmats cat [Vq*A : A in Wmats];
        if not &and[IsTrivialModGamma0(x*x, DN) : x in Gmats] then continue; end if;
        if not &and[IsTrivialModGamma0(x*y*x^-1*y^-1, DN) : x, y in Gmats] then continue; end if;
        if exists{i : i, j in [1..#Gmats] | i lt j and IsTrivialModGamma0(Gmats[i]*Gmats[j]^-1, DN)} then
            continue;
        end if;
        inG := func<Q | exists{x : x in Gmats | IsTrivialModGamma0(M2Q!al_matrix(Q, DN)*x^-1, DN)}>;
        ...
        for N2 in als do
            ...
            for N1 in als do
                ...
                if AtkinLehnerMul(N1, N2, DN) in Wodd then
                    if inG(N1) or inG(N2) then continue; end if;   // w_N1, w_N2 must lie outside G
                    return true, Sprintf(
